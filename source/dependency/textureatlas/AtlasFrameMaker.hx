package dependency.textureatlas;

import dependency.textureatlas.JSONData.AnimationData;
import dependency.textureatlas.JSONData.AtlasData;
import dependency.textureatlas.displayobject.SpriteAnimationLibrary;
import dependency.textureatlas.displayobject.SpriteMovieClip;
...
/**
 * Solution for Working with (JSON) Atlas Spritesheets from Adobe Animate (ONLY 2018);
 */
class AtlasFrameMaker extends FlxFramesCollection
{
	 ...
	 */
        public static function construct(key:String, folder:String = 'images', ?_excludeArray:Array<String> = null):FlxFramesCollection
        {
                var frameCollection:FlxFramesCollection;
                var frameArray:Array<Array<FlxFrame>> = [];
                var animationData:AnimationData = Json.parse(Paths.getTextFromFile('$folder/$key/Animation.json'));
                var atlasData:AtlasData = Json.parse(Paths.getTextFromFile('$folder/$key/spritemap.json').replace("\uFEFF", ""));//FIXED UTF8 w/ BOM error
                var graphicData:FlxGraphic = Paths.image('$key/spritemap', '$folder');
                var ss = new SpriteAnimationLibrary(animationData, atlasData, graphicData.bitmap);
                var t = ss.createAnimation();
                if(_excludeArray == null)
                        _excludeArray = t.getFrameLabels();
                frameCollection = new FlxFramesCollection(graphicData,FlxFrameCollectionType.IMAGE);
